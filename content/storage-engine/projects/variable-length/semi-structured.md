  ...
  **Source**: `src/backend/executor/json_utils.rs`
  ...
  Create a table with a JSON column (menu option **5**):
  
  ```
  Enter table name: users
  Enter column (name:type): id:INT
  Enter column (name:type): name:TEXT
  Enter column (name:type): metadata:JSON
  Enter column (name:type):
  ```
  
  Load `examples/example_json.csv` (menu option **6**):
  
  ```
  Enter table name: users
  Enter CSV path: examples/example_json.csv
  ```
  
  Display rows (menu option **7**):
  
  ```
  Enter table name: users
  
  === Tuples in 'mydb.users' ===
  Tuple 1: id=1 name='alice' metadata={"role": "admin", "level": 5}
  Tuple 2: id=2 name='bob' metadata={"role": "user", "level": 2}
  Tuple 3: id=3 name='carol' metadata={"role": "editor", "tags": ["news", "tech"]}
  Tuple 4: id=4 name='dave' metadata={"role": "user", "level": 1, "prefs": {"theme": "dark"}}
  Tuple 5: id=5 name='eve' metadata={"role": "admin", "level": 10}
  ```
  
  The JSON payload prints byte-for-byte as it was loaded — whitespace and key order are preserved.
  ...
  **Source**: `src/backend/executor/jsonb.rs`
  ...
  Create a table with a JSONB column (menu option **5**):
  
  ```
  Enter table name: profiles
  Enter column (name:type): id:INT
  Enter column (name:type): name:TEXT
  Enter column (name:type): profile:JSONB
  Enter column (name:type):
  ```
  
  Load `examples/example_jsonb.csv` (menu option **6**):
  
  ```
  Enter table name: profiles
  Enter CSV path: examples/example_jsonb.csv
  ```
  ...
  === Tuples in 'mydb.profiles' ===
  Tuple 1: id=1 name='alice' profile={"age":30,"city":"NYC"}
  Tuple 2: id=2 name='bob' profile={"age":25,"city":"LA","scores":[88,92,75]}
  Tuple 3: id=3 name='carol' profile={"active":true,"age":40}
  Tuple 4: id=4 name='dave' profile={"age":22,"city":"Chicago","meta":{"joined":"2024"}}
  Tuple 5: id=5 name='eve' profile={"active":false,"age":35,"city":"Seattle"}
  ```
  
  Notice that the keys come back **sorted lexicographically** (`active` before `age`, `age` before `city`) and there is no whitespace — the CSV input was reparsed and reserialized through the JSONB binary format.
  ...
  **Source**: `src/backend/executor/xml_utils.rs`
  ...
  Create a table with an XML column (menu option **5**):
  
  ```
  Enter table name: settings
  Enter column (name:type): id:INT
  Enter column (name:type): name:TEXT
  Enter column (name:type): config:XML
  Enter column (name:type):
  ```
  
  Load `examples/example_xml.csv` (menu option **6**):
  
  ```
  Enter table name: settings
  Enter CSV path: examples/example_xml.csv
  ```
  ...
  === Tuples in 'mydb.settings' ===
  Tuple 1: id=1 name='alice' config=<config><theme>dark</theme><lang>en</lang></config>
  Tuple 2: id=2 name='bob' config=<config><theme>light</theme><lang>fr</lang></config>
  Tuple 3: id=3 name='carol' config=<config><theme>dark</theme><notifications><email>true</email><sms>false</sms></notifications></config>
  Tuple 4: id=4 name='dave' config=<config><theme>auto</theme><lang>de</lang></config>
  Tuple 5: id=5 name='eve' config=<config><theme>light</theme><lang>en</lang><plugins><plugin name="spell"/></plugins></config>
  ```
  
  Like JSON, the original XML text is returned verbatim — only well-formedness is checked at insert time.
  ...
  **Source**: `src/backend/catalog/types.rs`, `src/backend/catalog/catalog.rs`
  ...
  First, register the UDT itself (menu option **9**):
  ...
  Enter field (name:type): zip:INT
  Enter field (name:type): street:TEXT
  Enter field (name:type): active:BOOLEAN
  Enter field (name:type):
  ```
  
  Create a table that uses the type as a column (menu option **5**):
  
  ```
  Enter table name: people
  Enter column (name:type): id:INT
  Enter column (name:type): name:TEXT
  Enter column (name:type): address:UDT:address
  Enter column (name:type):
  ```
  
  Load `examples/example_udt.csv` — fields are comma-separated within the column value, in the order they were declared on the type (menu option **6**):
  
  ```
  Enter table name: people
  Enter CSV path: examples/example_udt.csv
  ```
  ...
  === Tuples in 'mydb.people' ===
  Tuple 1: id=1 name='alice' address=(zip=101, street=MainSt, active=true)
  Tuple 2: id=2 name='bob' address=(zip=202, street=ElmAve, active=true)
  Tuple 3: id=3 name='carol' address=(zip=303, street=OakBlvd, active=false)
  Tuple 4: id=4 name='dave' address=(zip=404, street=PineLn, active=true)
  Tuple 5: id=5 name='eve' address=(zip=505, street=MapleRd, active=false)
  ```
  
  UDT values render as `(field=value, ...)`, with each field formatted according to its declared primitive type.
  ...
  **Source**: `src/backend/toast/mod.rs`, `src/backend/toast/toast_writer.rs`, `src/backend/toast/toast_reader.rs`, `src/backend/toast/compression.rs`
  ...
  The `Filter Tuples` command (menu option **11**) runs a sequential scan that materializes each tuple into typed datums and evaluates a single predicate against it. Only rows for which the predicate evaluates to **TRUE** are printed; **FALSE** and **UNKNOWN** (NULL) drop, matching SQL `WHERE` semantics.
  
  ### Supported predicates
  
  Phase 1 supports a single comparison: `=` or `!=` between a column (or a JSONPath into a JSON / JSONB column) and a literal.
  
  ```
  Predicate ::= Cmp(Expr, ('=' | '!='), Expr)
              | IsNull(Expr)1
  
  Expr      ::= Column(idx)
              | JsonPath(idx, path)     // JSON / JSONB columns only
              | Literal(Datum)
  ```
  
  `Datum` carries `Null`, `Int`, `Text`, `Bool`, `Number`, `Json`, or `JsonText`. Comparisons unify across kinds:
  
  - `Int` vs `Number` compares numerically.
  - `Text` vs `Number` / `Int` parses the text side once and compares numerically — useful when a JSONPath returns a stringy `"2024"` against a numeric literal.
  - `Text` vs `Text` trims trailing TEXT-column padding before comparing.
  - Either side `Null` produces UNKNOWN.
  
  **Source**: `src/backend/executor/predicate.rs`, `src/backend/executor/seq_scan.rs`
  
  ### Materialization
  
  Before evaluation, each tuple is decoded into a `Vec<Datum>`:
  
  | Column type | Datum produced |
  |-------------|---------------|
  | INT | `Datum::Int` |
  | TEXT | `Datum::Text` (trimmed) |
  | BOOLEAN | `Datum::Bool` |
  | JSON | `Datum::JsonText` (raw text; parsed lazily when a JSONPath is applied) |
  | JSONB | `Datum::Json` (decoded from binary into `serde_json::Value`) |
  | XML | `Datum::Text` |
  | UDT | `Datum::Text` (display string `(field=value, ...)`) |
  
  JSON columns stay as text in the common case where the predicate compares the whole value. They are parsed on demand only when a `JsonPath` expression forces it.
  
  ### JSONPath on JSON / JSONB
  
  For JSON or JSONB columns the CLI prompts for an optional JSONPath (`serde_json_path`). Leaving it blank compares the whole value; supplying a path extracts a single node (`path.query(&value).first()`):
  
  - Scalars unwrap into the matching `Datum` (`Bool`, `Number`, `Text`).
  - Arrays / objects stay as `Datum::Json`.
  - A path that matches nothing produces `Datum::Null` (predicate evaluates to UNKNOWN).
  
  The literal you type is then coerced — for path predicates, `true` / `false` become `Bool`, anything that parses as a float becomes `Number`, otherwise `Text`. For plain column predicates the literal is coerced according to the column's declared type.
  
  ### CLI walkthrough — JSON path equality
  
  Using the `users` table from the JSON walkthrough above:
  ...
  Columns:
    ...
    2. name (TEXT)
    3. metadata (JSON)
  Pick column number: 3
  JSONPath (e.g. $.score, blank for whole value): $.role
  Operator (= or !=): =
  Value: admin
  ...
  ### CLI walkthrough — JSONB nested path
  
  Using the `profiles` table from the JSONB walkthrough:
  
  ```
  Enter your choice: 11
  Enter table name: profiles
  Pick column number: 3
  JSONPath (e.g. $.score, blank for whole value): $.meta.joined
  Operator (= or !=): =
  Value: 2024
  ...
  The stored value is the JSON string `"2024"`, but mixed `Text` ↔ `Number` comparison parses it numerically, so the literal `2024` matches.
  
  ### CLI walkthrough — plain column
  
  `!=` works on any column. JSONPath is skipped for non-JSON columns:
  
  ```
  Enter your choice: 11
  Enter table name: users
  Pick column number: 2
  Operator (= or !=): !=
  Value: alice
  1
  Tuple 2: id=2 name='bob' ...
  Tuple 3: id=3 name='carol' ...
  Tuple 4: id=4 name='dave' ...
  Tuple 5: id=5 name='eve' ...
  Matched: 4
  ```
